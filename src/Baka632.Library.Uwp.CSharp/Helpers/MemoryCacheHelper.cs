...
public sealed class MemoryCacheHelper<T>
{
    ...
    /// </summary>
    /// <param name="key">访问缓存数据的键</param>
    /// <param name="value">缓存数据；若能够使用 <paramref name="key"/>，则返回实际数据，否则返回 <typeparamref name="T"/> 的默认值</param>
    ...
    /// <returns>指示过程是否成功的值</returns>
    /// <exception cref="ArgumentNullException"><paramref name="predicate"/> 为空</exception>
